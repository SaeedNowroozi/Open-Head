...
Ruler::Ruler(QGraphicsItem *parentItem) :
...
void Ruler::paint(QPainter *painter)
{
   ...
   // painter->fillRect(0, 0, width(), height(), mBackgroundColour);
    ...
    if (horizontal) {
        ...
        for (int x = startX; x < 10 + lvl1BaseSpacing; x += lvl2Spacing, ++tickmarkIndex, number += lvl2BaseSpacing) {
        ...
        for (int y = startY; y < 10/*height() */+ lvl1BaseSpacing; y += lvl2Spacing, ++tickmarkIndex, number += lvl2BaseSpacing) {
            if (tickmarkIndex % 5 == 0) {
...
//    painter->fillRect(0, 0, horizontal ? width() : 1, horizontal ? 1 : height(), lineColour);
    painter->fillRect(horizontal ? 0 : 10 /*width()*/ - 1, horizontal ?10 /*height()*/ - 1 : 0, horizontal ? 10/*width()*/ : 1, horizontal ? 1 : 10, lineColour);
